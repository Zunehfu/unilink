...
const get_user_posts_stmt = `SELECT * FROM posts WHERE user_id = ? AND is_anonymous = 0 LIMIT 10 OFFSET ?`;
...
const getPostsByUserId = async (req, res, next) => {
    try {
        ...
        const [posts] = await pool.query(get_user_posts_stmt, [userId, offset]);

        res.json(response(true, "GET_USER_POSTS", posts));
    } catch (err) {
        if (err instanceof Err) {
            res.json(response(false, err.message, {}));
        } else {
